...
class Model(object):
    def __init__(self, n_blocks, n_q_per_block, n_y_multiplier, n_theta, A_nnz_per_row):
        ...
        for i in range(n_y_multiplier):
            self.A.set_block(
                i,
                0,
                get_random_n_diagonal_matrix(
                    n=n_q_per_block, nnz_per_row=A_nnz_per_row
                ),
            )
        self.A = self.A.tocoo()
    ...
    def build_kkt(self):
        ...
        kkt.set_block(
            self.n_blocks, self.n_blocks, coo_matrix((self.theta.size, self.theta.size))
        )
